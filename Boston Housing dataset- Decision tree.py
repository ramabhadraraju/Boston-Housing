...
for depths in depth:
        for splits in split:
            X_train, X_test, Y_train, Y_test = cross_validation.train_test_split(X, Y,test_size = splits, random_state=99)
...
for tdepth in range(1,13):
    X_train, X_test, Y_train, Y_test = cross_validation.train_test_split(X, Y, random_state=99)
